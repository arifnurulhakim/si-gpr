...
@section('title', 'Edit Data Kas - E-Kartu Keluarga')
        ...
        <div class="border-t border-gray-200">
            <dl>
                ...
                <div class="bg-white px-3 py-4 sm:px-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt class="text-sm font-medium text-gray-500">Keluarga</dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $cashRecord->family->family_card_number }} - {{ $cashRecord->family->head_of_family_name }}</dd>
                </div>
            </dl>
        </div>
    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="family_id" id="family_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('family_id') border-red-300 @enderror">
                            ...
                            @foreach($families as $family)
                                <option value="{{ $family->id }}" {{ old('family_id', $cashRecord->family_id) == $family->id ? 'selected' : '' }}>
