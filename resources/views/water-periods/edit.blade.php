...
@section('title', 'Edit Periode Air - E-Kartu Keluarga')

@section('content')
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Periode Air</h1>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form method="POST" action="{{ route('water-periods.update', $period->id) }}" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="period_name" id="period_name" value="{{ old('period_name', $period->period_name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="period_code" id="period_code" value="{{ old('period_code', $period->period_code) }}"
                ...
                <div>
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $period->due_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('status') border-red-300 @enderror" required>
                ...
                <div>
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="number" name="price_per_m3" id="price_per_m3" value="{{ old('price_per_m3', $period->price_per_m3) }}"
                ...
                <div>
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="number" name="admin_fee" id="admin_fee" value="{{ old('admin_fee', $period->admin_fee) }}"
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-md p-4">
                <h3 class="text-sm font-medium text-gray-900 mb-3">Info Tambahan</h3>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-xs font-medium text-gray-500">Dibuat Oleh</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $period->createdBy->name }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500">Dibuat Pada</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $period->created_at->format('d M Y H:i') }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500">Total Record</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $period->waterUsageRecords->count() }} record</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500">Terakhir Diupdate</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $period->updated_at->format('d M Y H:i') }}</p>
                    </div>
                </div>
            </div>
